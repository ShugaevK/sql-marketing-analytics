...
Поля таблицы:
- **user_id** - Уникальный идентификатор пользователя. Примарный ключ.
- **registration_date** - Дата регистрации уже. Используется для когортного анализа.
- **source** - Источник трафика (органичный, но поиск, реферрал). Определяет эффективность канала.
- **device** - Тип девайса (дексктоп, мобиль, планшет). Нужен для анализа удобства.
- **region** - Географическая регион пользователя. Помогает регионализировать компании.
```
...
Поля таблицы:
- **click_id** - От всех кликов. Примарный ключ.
- **user_id** - Форейн ключ на таблица users. Привязывает клик к пользователю.
- **click_date** - Дата и время клика. Нужна для анализа трендов по дням.
- **campaign_id** - Идентификатор рекламной кампании. Группирует клики по кампаниям.
- **cost** - Стоимость клика (рекламное расходы). Конечно нужна для экономики.
- **device** - Тип трафика может отличаться в зависимости от девайса.
```
...
Поля таблицы:
- **conversion_id** - От всех конверсий. Примарный ключ.
- **user_id** - Привязывает конверсию к пользователю. Для того, чтобы знать что пкупил.
- **conversion_date** - Когда была конверсия (выкупка, регистрация, контакт). Нужна для CTR
- **revenue** - Стоимость конверсии (средняя прибыль). Нужна для рассчета ROAS.
- **conversion_type** - Тип конверсии (трял, купля, упгрейд). Нужна для сегментации.
```
...
Найти количество кликов и уникальных пользователей по дням.
Нюжно для: анализа трендов трафика, роста или спада.
...
Рассчитываем стоимость одного привлеченного клиента. Нюжно для расчета еффективности кампании.
Контролируем: Расходы / Конверсии = CPA
...
Анализируем, как рост пользователей в разные недели в структуре по источнику.
Нюжно для: выделения загрузки источников и равномерная аллокация бюджета.
